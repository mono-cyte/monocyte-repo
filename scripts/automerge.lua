function _get_autoupdate_pr_list()
    local result = {}
    local list = os.iorun("gh pr list --label auto-update --state open -R mono-cyte/monocyte-repo")
    if list then
...
function _check_pr_passed(id)
    local ok = os.vexecv("gh", { "pr", "checks", id, "-R", "mono-cyte/monocyte-repo" }, { try = true })
    if ok == 0 then
        return true
    end
end
...
function main()
    ...
    for _, info in ipairs(pr_list) do
        ...
        if _check_pr_passed(id) then
            ...
            os.vexec("gh pr merge %d --squash -d -R mono-cyte/monocyte-repo", id)
        end
    end
end
