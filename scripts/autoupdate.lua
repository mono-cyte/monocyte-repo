...
function _is_pending(instance, version)
    ...
    local repourl = "git@github.com:mono-cyte/monocyte-repo.git"
...
function _update_version(instance, version, shasum)
    ...
    if inserted then
        ...
        os.vexec(
            "gh pr create --label \"auto-update\" --title \"Auto-update %s to %s\" --body \"%s\" -R mono-cyte/monocyte-repo -B dev -H %s",
    ...
    while count < maxcount and #instances > 0 do
        ...
        if os.isfile(checkupdate_filepath) then
            ...
            if version and shasum and not _is_pending(instance, version) then
                cprint("package(%s): new version ${bright}%s${clear} found, shasum: ${bright}%s", instance:name(),
                    version, shasum)
